<template>
  <div class="grid grid-rows-3 grid-cols-3 p-4 rounded-lg m-auto border-2 border-slate-400/70 w-3/4 h-1/6">
    <div class="row-start-1 row-span-1 col-start-1 col-span-1 flex gap-2">
      <span class="py-1 px-2 border-2 border-red-500 text-red-500 font-semibold rounded-md uppercase h-max" v-for="tag in tags" :key="tag">{{tag}}</span>
    </div>
    <span class="row-start-1 row-span-1 col-start-3 col-span-1 update"><em>Last Updated &nbsp;</em> {{ lastUpdate }}</span>
...
export default defineComponent({
  ...
  computed: {
    lastUpdate(){
      return formatDistance(this.lastUpdateDate, new Date(), { addSuffix: true })
    }
  },
  methods: {
  },
